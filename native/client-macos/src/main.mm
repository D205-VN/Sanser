...
void printHelp() {
  std::cout
    << "sanser-client-macos --probe\n"
    << "sanser-client-macos --decode-snv capture_h264.snv [--max-packets N]\n"
    << "sanser-client-macos --listen-snv PORT [--max-packets N]\n"
    << "sanser-client-macos --listen-render-snv PORT [--control-port PORT] [--audio-port PORT] [--audio-jitter-ms N] [--max-packets N] [--session-token TOKEN] [--log-input] [--fullscreen] [--hide-cursor] [--relative-mouse]\n"
    << "sanser-client-macos --metal-test [--seconds 5]\n"
    << "sanser-client-macos --clipboard-read\n"
    << "sanser-client-macos --clipboard-write \"text\"\n"
    << "\n"
    << "Phase 5 prototype:\n"
...
void maybeRequestDecoderRecoveryKeyframe(const char* transport,
                                          const DecodeSummary& summary,
                                          const VtH264Decoder& decoder,
  ...
  if (sent) {
    std::cout << "SNV1_DECODER_RECOVERY transport=" << (transport ? transport : "unknown")
              << " reason=" << reason
              << " skippedNoParameters=" << summary.skippedNoParameters
              << " skippedEmptySamples=" << summary.skippedEmptySamples
...
private:
  ...
  std::size_t bufferedBytesLocked() const {
    std::size_t bytes = 0;
    for (const auto& entry : buffer_) {
      if (entry.second.payload.size() > maxBufferedBytes_ - std::min(bytes, maxBufferedBytes_)) {
        return maxBufferedBytes_ + 1;
      }
  ...
  void pruneBoundsLocked() {
    while (!buffer_.empty() &&
           (buffer_.size() > maxQueuedPackets_ || bufferedBytesLocked() > maxBufferedBytes_)) {
      const std::uint64_t droppedSequence = buffer_.begin()->first;
  ...
  static constexpr std::size_t maxQueuedPackets_ = 128;
  static constexpr std::size_t maxBufferedBytes_ = 4 * 1024 * 1024;
  std::mutex mutex_;
  std::condition_variable condition_;
  ...
  @autoreleasepool {
    try {
      ...
      while (true) {
        ...
        const std::uint32_t maxFramesPerPacket = std::max<std::uint32_t>(1, sampleRate / 10);
      ...
      while (maxPackets == 0 || totalPackets < maxPackets) {
        ...
        try {
          ...
          if (summary.packets == 1 || summary.packets % 30 == 0) {
            maybeRequestDecoderRecoveryKeyframe("tcp-render",
                                                 summary,
                                                 decoder,
      ...
      while (maxPackets == 0 || summary.packets < maxPackets) {
        ...
        SnvPacket packet;
          ...
          if (summary.packets == 1 || summary.packets % 60 == 0) {
            maybeRequestDecoderRecoveryKeyframe("udp-render",
                                                 summary,
                                                 decoder,
                                                 lastNoParameterSkipped,
                                                 lastEmptySampleSkipped);
  ...
  try {
    ...
    if (argc == 2 && std::string_view(argv[1]) == "--capabilities-json") {
      std::cout << "{\"product\":\"Sanser\",\"version\":\"2.0.0\","
                   ...
                   "\"nativeSnv2\":true,\"h264DecoderImplementation\":true,"
    ...
    std::cerr << "sanser-client-macos error: " << error.what() << "\n";
    return 1;
  }
}
