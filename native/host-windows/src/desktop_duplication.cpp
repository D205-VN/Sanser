...
bool DesktopDuplicator::captureFrame(FrameBgra& frame, std::uint32_t timeoutMs) {
  ...
  bool released = false;
  auto releaseFrame = [&]() -> HRESULT {
    if (!released) {
      const HRESULT releaseHr = impl_->duplication->ReleaseFrame();
      released = true;
      if (FAILED(releaseHr)) {
        impl_->recoveryPending = true;
        impl_->nextRecoveryAttempt = std::chrono::steady_clock::now();
  ...
  try {
    ...
    unmapStaging();
    const HRESULT releaseHr = releaseFrame();
    if (FAILED(releaseHr)) {
      std::fprintf(stderr,
                   "Desktop duplication ReleaseFrame failed; scheduling recovery (%s).\n",
                   hresultMessage(releaseHr).c_str());
      return false;
    }
    return true;
  } catch (...) {
