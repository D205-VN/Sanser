  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--output-dir") {
      ...
      std::cout
        ...
        << "sanser-host-windows --pipe --fps 60\n"
        << "sanser-host-windows --list-encoders\n"
        << "sanser-host-windows --encode h264 --frames 300 --fps 60 --output-file captures/capture_h264.mp4\n"
        << "sanser-host-windows --encode-pipe h264 --fps 60 --packet-file native-captures/capture.snv\n"
        << "sanser-host-windows --encode-pipe h264 --fps 60 --tcp-connect 100.x.y.z:7777\n"
...
public:
  ...
  std::uint64_t retransmitGeneration() {
    ...
    return crypto.enabled ? crypto.generation : 0;
  }
...
int runEncodedPipeMode(DesktopDuplicator& duplicator, const Options& options) {
  ...
  StreamDimensions streamDimensions = fitStreamDimensions(
    duplicator.width(),
    duplicator.height(),
    options.streamWidth,
  ...
  while (capturedFrames < targetFrames) {
    ...
    if (mediaCrypto) {
      ...
      if (mediaSnapshot.enabled &&
	    ...
	    std::uint64_t staleNackPackets = 0;
	    ...
	    while (drainedNackRequests < maxNackRequestsDrainedPerFrame &&
	      ...
	      const auto enqueueNow = std::chrono::steady_clock::now();
	      const std::uint64_t wallNowMicros = unixMicros();
	      const std::uint64_t requestAgeMicros = wallNowMicros > receivedNack.requestedAtMicros
	        ? wallNowMicros - receivedNack.requestedAtMicros
	        : 0;
	      const auto requestAge = std::chrono::microseconds(requestAgeMicros);
	      if (requestAge > retransmitMaxAge) {
	        statsUdpNackExpired += receivedNack.packetIds.size();
	        droppedNackPackets += receivedNack.packetIds.size();
	        staleNackPackets += receivedNack.packetIds.size();
	        continue;
	      }
	      const std::uint64_t mediaGeneration = udpClient ? udpClient->retransmitGeneration() : 0;
	      const auto originallyQueuedAt = enqueueNow - requestAge;
	      for (const std::uint64_t packetId : receivedNack.packetIds) {
	        const auto duplicate = std::find_if(
	          pendingUdpRetransmits.begin(),
	          pendingUdpRetransmits.end(),
	          [&](const PendingUdpRetransmit& pending) {
	            return pending.packetId == packetId && pending.mediaGeneration == mediaGeneration;
	          });
	    ...
	    std::uint64_t expiredNackPackets = staleNackPackets;
	    ...
	    if (cacheMisses > 0 || expiredNackPackets > 0) {
	      requestKeyframe("udp-repair-deadline", 0, 0, cacheMisses + expiredNackPackets, 0);
	    }
    ...
    if (hasNetworkVideo &&
...
#endif
      {
        ...
        continue;
	...
	#ifdef _WIN32
	      const InputBounds nextInputBounds{
	      ...
	      bool inputBoundsChanged = false;
	      {
	        std::lock_guard<std::mutex> lock(gInputBoundsMutex);
	        inputBoundsChanged = gInputBounds.left != nextInputBounds.left
	          || gInputBounds.top != nextInputBounds.top
	          || gInputBounds.width != nextInputBounds.width
	          || gInputBounds.height != nextInputBounds.height;
	        gInputBounds = nextInputBounds;
	      }
	      if (inputBoundsChanged) {
	        std::cerr << "SNINPUT target bounds changed left=" << nextInputBounds.left
	                  << " top=" << nextInputBounds.top
	                  << " width=" << nextInputBounds.width
	                  << " height=" << nextInputBounds.height
	                  << "\n";
	      }

	      const StreamDimensions nextStreamDimensions = fitStreamDimensions(
	        frame.width,
	        frame.height,
	        options.streamWidth,
	        options.streamHeight);
	      if (nextStreamDimensions.width != streamDimensions.width
	          || nextStreamDimensions.height != streamDimensions.height) {
	        const StreamDimensions previousStreamDimensions = streamDimensions;
	        streamDimensions = nextStreamDimensions;
	        if (restartEncoder(currentAdaptiveBitrate, "display-geometry-change")) {
	          lastEncoderRestartAt = std::chrono::steady_clock::now();
	          pendingTimelineSkipMicros = 0;
	          hasLastStreamTimestamp = false;
	          requestKeyframe("display-geometry-change", 0, 0, 0, 0);
	          std::cerr << "SNV1_STREAM_GEOMETRY_CHANGED previous="
	                    << previousStreamDimensions.width << "x" << previousStreamDimensions.height
	                    << " next=" << streamDimensions.width << "x" << streamDimensions.height
	                    << " source=" << frame.width << "x" << frame.height
	                    << "\n";
	        } else {
	          streamDimensions = previousStreamDimensions;
	        }
	      }
  ...
  try {
    ...
    if (argc == 2 && std::string_view(argv[1]) == "--capabilities-json") {
      std::cout << "{\"product\":\"Sanser\",\"version\":\"2.0.0\","
                   ...
                   "\"nativeSnv2\":false,\"h264EncoderImplementation\":true,"
    ...
    std::cerr << "sanser-host-windows error: " << error.what() << "\n";
    return 1;
  }
}
